...
FROM ((
	...
	FROM inventory AS i
	JOIN inventory_outcrop AS io ON io.inventory_id = i.inventory_id
	...
	FROM inventory AS i
	JOIN inventory_shotpoint AS isp ON isp.inventory_id = i.inventory_id
	...
	SELECT jsonb_agg(jsonb_build_object(
		...
		FROM inventory AS i
		JOIN inventory_well AS iw ON iw.inventory_id = i.inventory_id
	...
	SELECT jsonb_agg(jsonb_build_object(
		...
		FROM inventory AS i
		JOIN inventory_well AS iw ON iw.inventory_id = i.inventory_id
		JOIN well_place AS wpl ON wpl.well_id = iw.well_id
	...
	SELECT jsonb_agg(jsonb_build_object(
		...
		FROM inventory AS i
		JOIN inventory_well AS iw ON iw.inventory_id = i.inventory_id
		JOIN well_region AS wr ON wr.well_id = iw.well_id
